function [ucm ws_wt] = contours2ucm(pb_oriented, fmt)
...
pb_oriented = 1000*pb_oriented; % ojo
...
% output
super_ucm = normalize_output(super_ucm / 1000); % ojo
...
% Watershed Line Drawings from global Pb
%
% syntax:
%   [ws_pb, ws_wt] = WLD(pb_oriented, h)
%
% description:
%   Weights watershed arcs by median gPb.
%
% arguments:
%   gPb : continuous global Pb in [0 1]
%   h : use h-minima of gPb [1 255]
%
% output:
%   ws_pb:     Watershed Line Drawing: Watershed arcs weighted by mean oriented pb value
%   ws_wt:     Watershed points weighted by oriented pb value
%
% Pablo Arbelaez <arbelaez@eecs.berkeley.edu>
% August 2008
...
% learned on trainval bsds500
[tx, ty] = size(pb);
beta = [-2.7487; 11.1189];
pb_norm = pb(:);
x = [ones(size(pb_norm)) pb_norm]';
pb_norm = 1 ./ (1 + (exp(-x'*beta)));
pb_norm = (pb_norm - 0.0602) / (1 - 0.0602); % OJO
